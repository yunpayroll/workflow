package com.fengcheyun.workflow.command;
...
public class RollbackTaskCmd extends TaskCmd implements Command<Integer> {
    ...
    /**
     * 传入参数为当前任务id，执行后执行器删除本次任务，将任务退回到前一个任务。
     * 
     * @param taskId
     */
    ...
    public void processTask() {
         ...
         */
        try {
//            String sql = "update ACT_HI_ACTINST set END_TIME_=?,DURATION_=?,ASSIGNEE_=? where ID_=?";
            ...
            Date now = new Date();
            state.setLong(1, Long.parseLong(historicActivityInstance.getId()));
...
//            state.setString(3, "rollback");
//            state.setLong(4, Long.parseLong(historicActivityInstance.getId()));
            state.executeUpdate();
            state.close();
    ...
    public TaskEntity processPreTask() {
        //增加前任务节?
        TaskEntity preTask = (TaskEntity)Context.getCommandContext().getProcessEngineConfiguration().getTaskService().newTask();
...
//              String sql = "update ACT_HI_ACTINST set END_TIME_=?,DURATION_=?,ASSIGNEE_=? where ID_=?";
              ...
              PreparedStatement state = Context.getCommandContext()
                      ...
                      .prepareStatement(sql);
              Date now = new Date();
              state.setLong(1, Long.parseLong(hai.getId()));
//              state.setTimestamp(1, new java.sql.Timestamp(now.getTime()));
//              state.setLong(2, now.getTime()
//                      - historicActivityInstance.getStartTime().getTime());
//              state.setString(3, "rollback");
